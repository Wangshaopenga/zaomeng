...
# 击退目标
func knockback(origin,target,knockback_mp,height = 0):
	var knockback_direction = (target.global_position - origin.global_position).normalized()
	var knockback_force = knockback_direction * knockback_mp
	if height != 0:
...
#添加按键绑定
func addSkillButton(skill:Skill):
	if key_map.has(skill.bindKye):
		current_key = key_map[skill.bindKye]
	else:
		input_ui.addSkillButton(skill)
