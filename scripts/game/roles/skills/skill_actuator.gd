...
##设置技能执行几个阶段的值（按顺序），如果不设置则在 [method add_skill] 的时候添加的数据的时候没有执行时间
...
## 获取技能执行到的阶段。如果没有在执行则返回 [code]-1[/code]，如果没有这个技能则返回 [code]-2[/code]
func get_skill_stage(skill_name: StringName) -> int:
	var skill: TimeLine = get_skill(skill_name)
	if skill:
		return skill.get_current_stage()
	return NON_EXISTENT
