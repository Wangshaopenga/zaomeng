...
func _ready():
	skill_panel.scale *= target_zoom
	set_process(false)
	center_panel()
	target_offset = skill_panel.position
...
func center_panel():
	...
	var panel_size = skill_panel.size * target_zoom               # 缩放后 Panel 大小
	skill_panel.position = viewport_center - panel_size / 2    

# 缩放画布
func zoom_canvas(factor: float, mouse_pos: Vector2):
	# 获取当前的缩放位置
	var panel_size = skill_panel.size * skill_panel.scale
	var panel_center = skill_panel.position + panel_size / 2
	...
	# 计算新的缩放后中心
	var new_panel_size = skill_panel.size * target_zoom
	var new_panel_center = mouse_pos - offset

	# 通过缩放后的中心位置调整 Panel 的位置
	skill_panel.scale = Vector2(target_zoom, target_zoom)
	skill_panel.position = new_panel_center - new_panel_size / 2

# 更新每帧逻辑
func _process(delta):
	# 插值缩放
	zoom = lerp(zoom, target_zoom, move_speed * delta)
	skill_panel.scale = Vector2(zoom, zoom)
	...
	skill_panel.position = current_offset
