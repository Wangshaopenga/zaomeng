...
func init() -> void:
	...
	stats.hp_changed.connect(update_hp_bar)
	update_hp_bar()

	stats.mp_changed.connect(update_mp_bar)
	update_mp_bar()

	stats.exp_changed.connect(update_exp_bar)
	update_exp_bar()

	stats.hp_changed.connect(update_hp_regen_label)
	update_hp_regen_label()

	stats.mp_changed.connect(update_mp_regen_label)
	update_mp_regen_label()

	stats.player_death.connect(death)
	stats.upgrade.connect(upgrade)

	tree_exited.connect(func():
			stats.hp_changed.disconnect(update_hp_bar)
			stats.mp_changed.disconnect(update_mp_bar)
			stats.exp_changed.disconnect(update_exp_bar)
			stats.player_death.disconnect(death)
			stats.upgrade.disconnect(upgrade)
			stats.hp_changed.disconnect(update_hp_regen_label)
			stats.mp_changed.disconnect(update_mp_regen_label)
...
func update_hp_regen_label() -> void:
	hp_regen_label.visible = stats.hp_regen > 0
	hp_regen_label.text = str(stats.hp_regen) + "/s"


func update_mp_regen_label() -> void:
	mp_regen_label.visible = stats.mp_regen > 0
	mp_regen_label.text = str(stats.mp_regen) + "/s"
